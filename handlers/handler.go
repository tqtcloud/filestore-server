...
func DownloadHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Descrption", "attachment;filename=\""+fm.File_name+"\"")
	w.Write(data)
}
...
func FileMetaUpdateHandier(w http.ResponseWriter, r *http.Request) {
	...
	//dir,_:=os.Getwd()
	curFileMeta.File_name = newFileName
	meta1 := meta.FileMeta{
		FileShah: curFileMeta.File_sha1,
	...
	meta.UpdateFileMeta(meta1) //更新元数据
...
func FileDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(fMeta.File_addr)

	err := os.Remove(fMeta.File_addr)
	if err == nil {
		log.Printf("文件已删除：%s", fMeta.File_addr)
	} else {
		fmt.Println(err)
	}
